...
sequenceDiagram
    ...
    Note over V: 0
    A->>V: 1. Read expected = 0
    Note right of A: 2. Calculate new = 1
    B->>V: 3. Read expected = 0
    Note right of B: 4. Calculate new = 1
    A->>V: 5. CAS(0,1) ✓
    Note over V: 1
    B->>V: 6. CAS(0,1) ✗
    B->>V: 7. Read expected = 1
    Note right of B: 8. Calculate new = 2
    B->>V: 9. CAS(1,2) ✓
    Note over V: 2
```
...
```mermaid
graph TD
    Read["Read Current"] --> Calc["Calculate New"]
    Calc --> Cas{"CAS"}
    Cas -->|Success| Return
    Cas -->|Failure| Retry["Retry"]
    Retry --> Read
```
...
graph TD
    subgraph sync["synchronized"]
        S1["获取独占权限"] --> S2["只允许一个线程进入"]
    end
    subgraph cas["CAS"]
        C1["先计算，再验证旧值"] --> C2["失败则放弃并重试"]
    end
```
...
graph TD
    Read --> Calculate
    Calculate --> Validate
    Validate --> Commit["Commit or Retry"]
```
...
graph TD
    subgraph Lock["Lock Based"]
        L1["Thread A 拥有锁"] --> L2["Thread B 等待释放"]
    end
    subgraph CAS["CAS Based"]
        C1["Thread A 更新值"] --> C2["Thread B 检测旧值"]
    end
```
...
```mermaid
graph LR
    Read --> Calculate --> CAS["CAS Success"]
```
...
graph LR
    ...
    subgraph B["Thread B"]
        B1["Read"] --> B2["CAS ✗"] --> B3["Read"] --> B4["CAS ✗"] --> B5["Retry"]
    end
    subgraph C["Thread C"]
        C1["Read"] --> C2["CAS ✗"] --> C3["Read"] --> C4["CAS ✓"]
    end
    subgraph D["Thread D"]
        D1["Read"] --> D2["CAS ✗"] --> D3["Retry"]
    end
```
...
graph TD
    ...
    subgraph After_pop_A["After pop A"]
        top2["Top"] --> B2["Node B"]
        B2 --> C2["Node C"]
    end
    subgraph After_pop_B["After pop B"]
        top3["Top"] --> C3["Node C"]
    end
    subgraph After_push_A["After push A"]
        top4["Top"] --> A4["Node A"]
        A4 --> C4["Node C"]
    end
```
