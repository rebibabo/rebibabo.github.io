...
├──────┼──────────────────────────────┼──────────────────────────────┼───────────┤
│  1   │ Read expected = 0            │                              │     0     │
│  2   │ Calculate newValue = 1       │                              │     0     │
│  3   │                              │ Read expected = 0            │     0     │
│  4   │                              │ Calculate newValue = 1       │     0     │
│  5   │ CAS(0, 1) succeeds           │                              │     1     │
│  6   │                              │ CAS(0, 1) fails              │     1     │
│  7   │                              │ Read expected = 1            │     1     │
│  8   │                              │ Calculate newValue = 2       │     1     │
│  9   │                              │ CAS(1, 2) succeeds           │     2     │
└──────┴──────────────────────────────┴──────────────────────────────┴───────────┘
```
...
Thread A 拥有锁
        ↓
Thread B 不能进入临界区，必须等待 Thread A 释放锁


CAS Based
Thread A 更新值
        ↓
Thread B 先计算，再检测旧值是否变化
```
...
```text
Thread A: Read → CAS Success
Thread B: Read → CAS Failure → Read → CAS Failure → Retry
Thread C: Read → CAS Failure → Read → CAS Success
Thread D: Read → CAS Failure → Retry
```
