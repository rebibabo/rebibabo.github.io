...
```mermaid
graph TD
    A["Thread-A"] --> B["ThreadLocalMap"] --> C["Entry<br>key = ThreadLocal object<br>value = UserA"]
```
...
```mermaid
graph LR
    A["Thread-A"] --> B["ThreadLocalMap"] --> C["tl -> UserA"]
    D["Thread-B"] --> E["ThreadLocalMap"] --> F["tl -> UserB"]
```
...
```mermaid
graph TD
    A["第一次 set: tl -> userA"] --> B["第二次 set: tl -> userB"]
```
...
```mermaid
graph TD
    A["Thread-A 执行 Task-A"] --> B["Task-A 结束"] --> C["Thread-A 结束"] --> D["ThreadLocalMap 随 Thread-A 一起回收"]
```
...
```mermaid
graph TD
    A["Thread-1"] --> B["Task-A"] --> C["Task-B"] --> D["Task-C"]
```
...
```mermaid
graph TD
    A["外部变量 tl"] -- strong --> B["ThreadLocal 实例"]
    C["Entry.key"] -- weak --> B
```
...
```mermaid
graph TD
    A["Thread"] --> B["ThreadLocalMap"] --> C["Entry.key"] -- strong --> D["ThreadLocal 实例"]
```
...
graph TD
    subgraph "GC 之前"
        A1["Thread"] --> A2["ThreadLocalMap"] --> A3["Entry"]
        A3 --> A4["key: ThreadLocal 实例"]
        A3 --> A5["value: User 实例"]
    end
    subgraph "GC 之后"
        B1["Thread"] --> B2["ThreadLocalMap"] --> B3["Entry"]
        B3 --> B4["key: null"]
        B3 --> B5["value: User 实例"]
    end
```
...
```mermaid
graph TD
    A["Thread"] --> B["ThreadLocalMap"] --> C["Entry"] --> D["value"]
```
...
```mermaid
graph TD
    A["ThreadLocal key 被 GC 回收"] --> B["Entry.key 变成 null"] --> C["Entry.value 仍然存在"] --> D["等待后续 get/set/remove/rehash 时顺带清理"]
```
