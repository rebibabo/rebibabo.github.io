...
```text
┌──────────┐        ┌────────────────┐        ┌────────────────────┐
│ Thread-A │ ────→  │ ThreadLocalMap │ ────→  │ tl -> UserA        │
└──────────┘        └────────────────┘        └────────────────────┘

┌──────────┐        ┌────────────────┐        ┌────────────────────┐
│ Thread-B │ ────→  │ ThreadLocalMap │ ────→  │ tl -> UserB        │
└──────────┘        └────────────────┘        └────────────────────┘
```
...
Thread-A 结束
    ↓
ThreadLocalMap 随 Thread-A 一起回收
```
...
ThreadLocalMap
  ↓
Entry.key ──strong──→ ThreadLocal object
```
...
```text
ThreadLocal key 被 GC 回收
    ↓
Entry.key 变成 null
    ↓
Entry.value 仍然存在
    ↓
等待后续 get/set/remove/rehash 时顺带清理
```
