...
```mermaid
graph TD
    D["data = 42"] -->|Program Order| R["ready = true"]
    R -->|Volatile Rule| RR["read ready == true"]
    RR -->|Program Order| RD["read data"]
```
...
```mermaid
graph TD
    Read["Read count"] --> Calc["Calculate count + 1"]
    Calc --> Write["Write count"]
```
...
sequenceDiagram
    ...
    A->>V: 1. Read 0
    B->>V: 2. Read 0
    Note over A: 3. Calculate 1
    Note over B: 3. Calculate 1
    A->>V: 4. Write 1
    Note over V: 1
    B->>V: 5. Write 1
    Note over V: 1
...
```mermaid
graph TD
    Alloc["Allocate memory"] --> Init["Initialize object"]
    Init --> Assign["Assign reference"]
```
