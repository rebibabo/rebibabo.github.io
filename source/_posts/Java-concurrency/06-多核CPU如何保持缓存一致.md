...
graph LR
    subgraph Core1["Core 1"]
        direction LR
        L1a["L1 Cache: count = 0"]
    end
    subgraph Core2["Core 2"]
        direction LR
        L1b["L1 Cache: count = 0"]
    end
    L1a --> Mem["Main Memory: count = 0"]
    L1b --> Mem
```
...
```mermaid
graph LR
    A["Core 想要写入"] --> B["请求独占权限"]
    B --> C["将其他 Core 副本失效"]
    C --> D["修改自己的 Cache Line"]
    D --> E["写回主内存（可选）"]
```
...
graph LR
    subgraph Core1["Core 1"]
        direction LR
        S1["State: Shared<br>count = 0"]
    end
    subgraph Core2["Core 2"]
        direction LR
        S2["State: Shared<br>count = 0"]
    end    S1 --> S2
```
...
sequenceDiagram
    ...
    Note over C1,C2: 1. Both Shared, count = 0
    C1->>Bus: 2. Request ownership
    Bus->>C2: 3. Invalidate
    Note over C1: 4. Modified, count = 1
    Note over C2: Invalid
```
...
```mermaid
graph LR
    C1["Core 1: requests ownership"] --> Arb["Hardware Arbitration"]
    C2["Core 2: requests ownership"] --> Arb
    Arb --> Win["One request succeeds"]
    Win --> Invalid["Other copy → Invalid"]
```
...
```mermaid
graph LR
    Request["请求修改权限"] --> Compare["比较当前值与期望值"]
    Compare -->|相等| Write["写入新值"]
    Write --> Return["返回成功或失败"]
```
...
```mermaid
graph LR
    Base0["base = 0"] -->|increment| Base1["base = 1"]
```
...
```mermaid
graph LR
    A["Thread A"] --> Cell0["Cell 0"]
    B["Thread B"] --> Cell2["Cell 2"]
    C["Thread C"] --> Cell1["Cell 1"]
    D["Thread D"] --> Cell3["Cell 3"]
```
