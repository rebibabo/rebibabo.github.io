...
```
①订单服务            ②Prometheus            ③Grafana
 埋点产生指标          定时来抓 + 存储          定时去查 + 画图
 /actuator/prometheus  （时序数据库）          （可视化大盘）
       │                   ▲   │                   │
       └───── 被抓取 ───────┘   └──── 被查询 ────────┘
```
...
```
Dashboard（一整块大盘）
  ├── Panel 1：下单 QPS          ← 背后是 Query: rate(order_create_total[1m])
  ├── Panel 2：下单成功率         ← 背后是另一行 PromQL
  ├── Panel 3：各渠道下单占比      ← 又一行 PromQL
  └── Panel 4：P99 耗时
        ↑
  所有 Panel 的数据，都来自同一个 Data Source（Prometheus）
```
...
每隔 1 分钟
   │
   ▼
跑一遍查询：下单失败率 = rate(order_create_failed[5m]) / rate(order_create_total[5m])
   │
   ▼
和阈值比：> 5% 吗？
   │
