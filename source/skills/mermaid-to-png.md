...
> ⚠️ **关于自动补连接**：脚本会为无连接的多子图自动补 `~~~`（隐性连接），子图内孤立节点也会用 `~~~` 串链。
> 这只解决布局问题，**不表达任何业务顺序关系**——`~~~` 是无箭头的不可见连接，仅让 Mermaid 引擎把它们排成一行。

```
    subgraph A[...] ... end
    subgraph B[...] ... end
    A_end_node ~~~ B_start_node    ← 用 ~~~ 隐性连接，不表达业务顺序
```
...
| `&#40;`/`&#41;` → `(`/`)` | 修复 HTML 转义 |
| 同一行多个 `-->` → 拆成多行 | 防止解析错误 |
| 子图内孤立节点 → `~~~` 串链（无箭头，不表达顺序） | 只解决布局，不改变语义 |
| 多子图无连接 → `~~~` 隐性连接（无箭头，不表达顺序） | 只解决布局，不改变语义 |
