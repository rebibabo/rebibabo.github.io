...
each Worker owns a Local Deque
  ↓
ForkJoinTask can be pushed, popped, stolen, executed, joined
```
...
pool.invoke(rootTask)
  ↓
ForkJoinPool accepts rootTask
  ↓
Worker-1 executes rootTask.compute()
  ↓
split into left and right
  ↓
left.fork()
  ↓
left enters Worker-1 Local Deque
  ↓
Worker-1 executes right.compute()
  ↓
left.join()
  ↓
...
if completed, return result
  ↓
if still local, execute it
  ↓
if stolen, help other work
  ↓
if no progress possible, wait
```
...
parallelism
  ↓
target active workers
  ↓
not strict maximum threads
```
