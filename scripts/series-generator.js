...
// Card color classes — cycled for each category
...
hexo.extend.generator.register('series-page', function(locals) {
  ...
  const dirs = fs.readdirSync(postsDir).filter(d => {
    const full = path.join(postsDir, d);
    return fs.statSync(full).isDirectory() && d !== '.obsidian';
  });
  ...
  dirs.forEach(dir => {
    ...
    // Read front matter of first post to get category slug
    let categorySlug = '';
    const firstContent = fs.readFileSync(path.join(dirPath, files[0]), 'utf8');
    const fm = parseFrontMatter(firstContent);
    if (fm.categories) {
      categorySlug = Array.isArray(fm.categories) ? fm.categories[0] : fm.categories;
    }
  ...
  // Build card HTML
  const cardsHtml = categories.map((cat, i) => {
    const colorClass = COLOR_CLASSES[i % COLOR_CLASSES.length];
    const icon = ICONS[i % ICONS.length];
    return `
  <a href="/categories/${cat.slug}/" class="series-card ${colorClass}">
...
.series-particles::before,
.series-particles::after {
  content: '';
  position: absolute;
  border-radius: 50%;
  opacity: 0.12;
...
[data-user-color-scheme="dark"] .series-card {
  background: rgba(255,255,255,0.04);
  border-color: rgba(255,255,255,0.08);
}

[data-user-color-scheme="dark"] .series-card:hover {
...
@media (max-width: 768px) {
  ...
  .series-card { padding: 24px 20px 20px; }
}
